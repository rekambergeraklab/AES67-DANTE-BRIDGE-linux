...
# Interactive: NIC selection, hardware/software PTP mode, master/slave, runtime channel count
# ==============================================================================
...
# ==============================================================================
# STEP 3: AES67 64-CHANNEL CONFIG TEMPLATE (SAP RX/TX, CHANNELS_PLACEHOLDER)
# ==============================================================================
echo "⏳ Step 3: Generating PipeWire AES67 config template (1–64 channels)..."

PW_DIR="$USER_HOME/.config/pipewire"
PW_TEMPLATE="$PW_DIR/aes67-template.conf"
...
context.modules = [
    ...
    # AES67 TRANSMITTER (TX & SAP ANNOUNCE)
    { name = libpipewire-module-rtp-sink
        args = {
            local.ifname = "${SELECTED_NIC}"
            sess.media = "audio"
            audio.rate = 48000
            audio.channels = CHANNELS_PLACEHOLDER
            audio.position = CHANNEL_ARRAY_PLACEHOLDER
            rtp.ptime = 1
            node.name = "AES67_TX_\${CHANNELS}ch"
            node.description = "AES67 Transmit (\${CHANNELS}ch)"
            sap.enabled = true
            sap.ip = "239.255.255.255"
            sap.port = 9875
            sess.name = "AES67 TX (\${CHANNELS}ch) from $REAL_USER"
        }
    }
]
EOF

chown $REAL_USER:$REAL_USER $PW_TEMPLATE
echo "✅ Step 3 Complete: PipeWire config template at $PW_TEMPLATE"

# ==============================================================================
# STEP 4: GENERATE THE ON-DEMAND LAUNCHER SCRIPT (USER SETS PTP ROLE, CHANNELS)
# ==============================================================================
echo "⏳ Step 4: Building the Session Launcher Script..."

LAUNCHER="$USER_HOME/start-aes67-session.sh"

cat << 'LAUNCH_EOF' > $LAUNCHER
#!/bin/bash

REAL_USER=${SUDO_USER:-$USER}
XDG_RUNTIME_DIR=/run/user/$(id -u $REAL_USER)
PW_TEMPLATE="$HOME/.config/pipewire/aes67-template.conf"

# Prompt for PTP Role (hardware mode only)
if grep -q "^hardware" <<<'PTP_MODE_PLACEHOLDER'; then
  echo ""
  echo "PTP Role for this session:"
  echo "1) Master (provide clock for network)"
  echo "2) Slave (sync clock from grandmaster)"
  read -rp "➡️  Enter 1 for master or 2 for slave [default: 2]: " PTP_ROLE_INPUT
...
# Prompt for Output Channels
echo ""
read -rp "➡️  Enter number of output channels to transmit (1-64, default: 8): " CHANNELS
if [[ -z "$CHANNELS" ]]; then CHANNELS=8; fi
if ! [[ "$CHANNELS" =~ ^[0-9]+$ ]] || (( CHANNELS < 1 || CHANNELS > 64 )); then
    echo "❌ Error: Invalid channel count."
    exit 1
fi

# Build audio.position array for PipeWire config syntax
POS_ARR="[ "
...
SESSION_CONF="/tmp/aes67-session.conf"
sed -e "s/CHANNELS_PLACEHOLDER/$CHANNELS/g" \
    -e "s|CHANNEL_ARRAY_PLACEHOLDER|$POS_ARR|g" \
    "$PW_TEMPLATE" > "$SESSION_CONF"
...
if grep -q "^hardware" <<<'PTP_MODE_PLACEHOLDER'; then
  ...
  echo "Selected PTP Role: $PTP_ROLE"
  if [[ "$PTP_ROLE" == "master" ]]; then
      echo "⏳ Starting ptp4l in MASTER mode..."
      ptp4l -i SELECTED_NIC_PLACEHOLDER -H -S -q -A -m &
      PTP_PID=$!
  else
      echo "⏳ Locking SELECTED_NIC_PLACEHOLDER to network grandmaster (SLAVE)..."
      ptp4l -i SELECTED_NIC_PLACEHOLDER -H -S -q &
      PTP_PID=$!
  fi
  sleep 2

  echo "⏳ Syncing system clock to SELECTED_NIC_PLACEHOLDER PTP hardware..."
  phc2sys -s SELECTED_NIC_PLACEHOLDER -c CLOCK_REALTIME -w -q &
  PHC_PID=$!
  sleep 3
...
echo "🎵 Launching PipeWire Engine with $CHANNELS channels..."
sudo -u $REAL_USER XDG_RUNTIME_DIR=$XDG_RUNTIME_DIR pipewire -c "$SESSION_CONF" &
PW_PID=$!
...
# Now patch in the variables the launcher script needs (PTP_MODE and NIC)
sed -i "s|PTP_MODE_PLACEHOLDER|$PTP_MODE|g" "$LAUNCHER"
sed -i "s|SELECTED_NIC_PLACEHOLDER|$SELECTED_NIC|g" "$LAUNCHER"
...
echo ""
echo " - You will pick master/slave and output channel count each time."
echo " - Press Ctrl+C in that terminal to turn everything off."
echo "============================================================"
